...
def get_data(filename):
    # make sure that the Cabin column is read as a string, not as numbers
    data = pd.read_csv(filename, dtype={ "Cabin": np.str })
    return data

# Name, Ticket, Cabin are unused at this time
...
class DerivedNumericalAttributesAdder( sklearn.base.BaseEstimator, sklearn.base.TransformerMixin ):
    ...
    def transform( self, X, y=None ):
        # Maybe survival rate depends on the square of the Age?
        # insert Age squared as a feature
        # X.insert(len(X.columns), "Age^2", X.loc[:,'Age'] * X.loc[:,'Age'])
        return X
...
class DerivedCategoricalAttributesAdder( sklearn.base.BaseEstimator, sklearn.base.TransformerMixin ):
    ...
    def transform( self, X, y=None ):
        # Maybe survival rate depends on the difference of SibSp and Parch?
        # insert SibSp - Parch as a feature
        # X.insert(len(X.columns),'SibSp-Parch', pd.Series(X.loc[:,'SibSp'] - X.loc[:,'Parch']))
        return X
